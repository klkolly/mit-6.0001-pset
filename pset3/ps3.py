...
def get_word_score(word, n):
    ...
    for i in word:
        part_a += SCRABBLE_LETTER_VALUES.get(i,0)

    part_b = 7*len(word) -3*(n - len(word))
    if part_b < 1:
        part_b = 1
...
def is_valid_word(word, hand, word_list):
    ...
    wildcard_isvalid = True


    #iswildcard  if not find = -1  if find =word[i] =='*'
    wildcard = word.find('*')
    ...
    if wildcard == -1:
        ...
        # for replce '*' with vowels
        wildcard_isvalid = False

        for i in VOWELS:
            wildcard_temp = word[:(wildcard)] + i + word[(wildcard+1):]
            if wildcard_temp in word_list:
                wildcard_isvalid = True
                break
    if not wildcard_isvalid:
        return False

    #is have hand
    for letter in word_dict.keys():
        if word_dict[letter] > hand_copy.get(letter,0):
            return False
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    HAND_SIZE = 7
    try:
        hand_num = int(input('Enter total nuber of hands: '))
    except ValueError:
    ...
    while hand_num > 0:
        ...
        replay = True
        while replay:
            cur_score = play_hand(hand, word_list)
            print('-'*10)
            replay_hand = input("would you like to replay the hand:(yes/no) ")
            if replay_hand.lower() == 'no':
                total_score += cur_score
                replay = False

        hand_num -= 1

        #finish  total num of hand
